(function () {
  ...
  function buildIcs(entries, firstMon, remind, calName, withTz) {
    ...
    var dtstamp = nowStamp();
    for (var i = 0; i < entries.length; i++) {
      var e = entries[i];
      if (!e.name || !e.st || !e.et) continue;
      var ws = parseInt(e.ws, 10) || 1;
      var we = parseInt(e.we, 10) || ws;
      var day = new Date(firstMon.getFullYear(), firstMon.getMonth(), firstMon.getDate());
      day.setDate(day.getDate() + (ws - 1) * 7 + (e.weekday - 1));
      var ymd = fmtDate(day);
      ...
      var count = we - ws + 1; if (count < 1) count = 1;
      var desc = "教学班代码：" + e.code + "\n教师：" + e.teacher + "\n周次：" + ws + "~" + we + "周";
      L.push("BEGIN:VEVENT");
      L.push("UID:" + (e.code || "ev") + "-" + (i + 1) + "@kejiantoics.local");
      L.push("DTSTAMP:" + dtstamp);
      L.push(dstart);
      L.push(dend);
      L.push("RRULE:FREQ=WEEKLY;COUNT=" + count);
  ...
  $("btnExport").onclick = function () {
    var entries = readRows().filter(function (e) { return e.name && e.st && e.et; });
    if (!entries.length) { alert("没有可导出的课程（请检查课程名与开始/结束时间）。"); return; }
